...
Original file is located at
    https://colab.research.google.com/drive/17Zupp0UvgMX8x8wcyYEHWopz9nCC5gq8
"""
...
def solve(Vertices,a):
    ...
    for i in range(len(Vertices)):
        A[i][i] += len(Vertices[i]._nbrs) + len(Vertices[i]._pinNbrs)*a
    ...
    #run_force_directed_placement(modules, nets, iterations=200)
...
def solveIter(V, bbox, outfile, d, Numiter,a):
...
def place(deffile, leffile, outfile,a, Numiter=4):
    ...
    plot(solveIter(V, bb, outfile, d, Numiter,a), bb)
    d.writeDEF(outfile)
    o = overlapCost(V)
    h = hpwlCost(d.nets(), Vdict, Pdict, V)
    t = (0.1*o+h)
    return (o, h, t)
...
x = []
for i in range(6,7):
    T = place('dma.def', 'dma.lef', 'dma_out.def',i,9)
    oCost.append(T[0])
    hCost.append(T[1])
    tCost.append(T[2])
    x.append(i)

print(oCost,hCost,tCost)